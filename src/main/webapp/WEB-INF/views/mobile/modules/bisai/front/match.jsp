						...
						<div>
							...
							<c:choose>
								<c:when test="${match.state==0 }">
									<span class="button_update"><a href="${ctx }/${frontPath}match/editMatch${urlSuffix}?id=${match.id}">编辑</a></span>
									<span class="button_shut"><a href="${ctx }/${frontPath}match/closeMatch${urlSuffix}?id=${match.id}">关闭</a></span>
								</c:when>
								<c:when test="${match.state==1 }">
									<c:if test="${empty match.changci or match.changci=='0'}">
										<span class="button"><a href="${ctx }/${frontPath}match/matchBm${urlSuffix}?type=1&id=${match.id}">单项报名</a></span>
									</c:if>
									<c:if test="${not empty match.changci and match.changci!='0'}">
										<span class="button"><a href="${ctx }/${frontPath}match/matchBm${urlSuffix}?type=2&id=${match.id}">团体报名</a></span>
									</c:if>
								</c:when>
								<c:when test="${match.state==2 }">
									<span class="button_update"><a href="${ctx }/${frontPath}match/matchScore${urlSuffix}?type=1&stype=0&id=${match.id}">查看成绩</a></span>
									<span class="button_shut"><a href="${ctx }/${frontPath}match/matchScore${urlSuffix}?type=1&stype=1&id=${match.id}">成绩录入</a></span>
								</c:when>
								<c:when test="${match.state==-1 }">
									<span class="button"><a href="${ctx }/${frontPath}match/matchRecreate${urlSuffix}?id=${match.id}">再次发起</a></span>
								</c:when>
							</c:choose>
						</div>
