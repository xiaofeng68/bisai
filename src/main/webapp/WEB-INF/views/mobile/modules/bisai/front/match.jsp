            ...
            <c:if test="${match.state>0 }">
                <li>
                    <a href="${ctx }/${frontPath}activity${urlSuffix}?id=${match.id}&isall=1">
                        ...
                        <div>
                            <span class="timell"><fmt:formatDate value="${match.starttime }"
                                                                 pattern="M月d日"/>-<fmt:formatDate
                                    value="${match.endtime }" pattern="M月d日"/></span>
                        </div>
                            ...
                            <c:choose>
                                <c:when test="${match.state==0 }">
                                    ...
                                    <div class="button_update fl"><a
                                            href="${ctx }/${frontPath}match/editMatch${urlSuffix}?id=${match.id}">编辑</a></div>
                                    <div class="button_shut fl"><a
                                            href="${ctx }/${frontPath}match/closeMatch${urlSuffix}?id=${match.id}">关闭</a></div>
                                    </div>
                                </c:when>
                                <c:when test="${match.state==1 }">
                                    <c:if test="${empty match.changci or match.changci=='0'}">
                                        <div class="button"><a
                                                href="${ctx }/${frontPath}match/matchBm${urlSuffix}?type=1&id=${match.id}">单项报名</a>
                                        </div>
                                    </c:if>
                                    <c:if test="${not empty match.changci and match.changci!='0'}">
                                        <div class="button"><a
                                                href="${ctx }/${frontPath}match/matchBm${urlSuffix}?type=2&id=${match.id}">团体报名</a></div>
                                    </c:if>
                                </c:when>
