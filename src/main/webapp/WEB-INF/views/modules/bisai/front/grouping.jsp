...
<body>
	<header class="grouping_header clearfix">
		<span class="fl"> <a
			href="${ctx }/${frontPath}match${urlSuffix}"> <img
				src="${ctxStaticFront }/images/r-arrow.png">
		</a>
		</span> <span>分组名单</span>
	</header>
	<section>
		<select id="typeSelect" onchange="initGroups()">
			<option value="">请选择</option>
			<c:forEach var="typeNode" items="${fns:getMatchTypeNote(matchid,type) }">
				<option value="${typeNode.id }" groups="${typeNode.peoples }">${fns:getDictLabels(typeNode.type,'MatchTypeNote_type','')}</option>
			</c:forEach>
		</select>
		<select id="groupSelect" onchange="targetDiv()">
			<option value="">请选择</option>
		</select>
	</section>
	<c:forEach var="typeNode" items="${fns:getMatchTypeNote(matchid,type) }">
